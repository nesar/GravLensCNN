...
glob.glob(DirIn + '*autoDenoise*200*hdf5')


# filelist = sorted(glob.glob(DirIn +'*.npy'))   # All
# hyperpara = '*opti1*lr5e-05*decay0.1*batch32*epoch200*'
# lr0.001_decay0.1_batch32_epoch200
hyperpara = '*32*200*'


filelistAuto = sorted(glob.glob(DirIn + '*auto*Denoise'+hyperpara + '*.hdf5'))
filelistEncode = sorted(glob.glob(DirIn + 'encodeDenoise'+hyperpara + '*.hdf5'))
filelistDecode = sorted(glob.glob(DirIn + 'decodeDenoise'+hyperpara + '*.hdf5'))
histlist = sorted(glob.glob(DirIn + '*Denoise' +hyperpara + '*.npy'))
...
if ifTest:
    ...
    #     num_epoch)
    #
    # loaded_model = load_model('ModelOutEncode/auto_' + fileOut + '.hdf5')
    # encoder = load_model('ModelOutEncode/encode_' + fileOut + '.hdf5')
    # decoder = load_model('ModelOutEncode/decode_' + fileOut + '.hdf5')
    #
    ...
    if plotSample:
        n = 4
        np.random.seed(2135)
        pltrange = np.random.randint(1000, size=n)
    ...
    if plotLoss:
        ...
        # history = np.load('ModelOutEncode/' + fileOut+'.npy')
